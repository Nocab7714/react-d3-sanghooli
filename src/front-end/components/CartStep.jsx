    ...
    <>
      ...
      <div className="bg-white rounded-4 overflow-hidden mb-10 mb-xl-19">
        ...
        <ul className="fs-7 d-flex d-lg-none py-5 mb-0 px-0 flex-wrap">
          {progressMap
            .filter((progressMap) => progressMap.step === step)
            .map((item) => (
              <ol
                key={item.step}
                className="cart-step d-flex align-items-center gap-4 px-6"
              >
                <span
                  className={`material-symbols-outlined rounded-circle p-4 fs-2 ${
                    item.step <= step
                      ? 'text-primary-dark bg-primary-light'
                      : 'text-neutral40 bg-neutral20'
                  }`}
                >
                  {item.icon}
                </span>
        ...
        <ul className="fs-7 d-none d-lg-flex py-5 mb-0 px-0 flex-wrap">
          {progressMap.map((progressMap) => (
            <ol
              key={progressMap.step}
              className="cart-step d-flex align-items-center gap-4 px-6"
            >
              <span
                className={`material-symbols-outlined rounded-circle p-4 fs-2 ${
                  progressMap.step <= step
                    ? 'text-primary-dark bg-primary-light'
                    : 'text-neutral40 bg-neutral20'
                }`}
              >
              ...
              <div
                className={`${
                  progressMap.step <= step ? '' : 'text-neutral40'
                }`}
              >
